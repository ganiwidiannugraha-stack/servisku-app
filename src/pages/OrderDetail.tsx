...
import { Printer, Wallet, MessageCircle, Plus, Edit2, Package, Trash2, User, MonitorSmartphone, Wrench, Phone, MapPin, BookOpen, FileText } from 'lucide-react';
      ...
      <div className="w-full mt-4">
        ...
        <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <h1 className="text-xl font-medium text-gray-900">Order #{order.noServis}</h1>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
          ...
          <div className="xl:col-span-2 space-y-6">
            ...
            {/* Row 1: Data Pelanggan & Perangkat */}
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 relative">
                ...
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-12 h-12 bg-slate-800 rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
                    <img src={`https://ui-avatars.com/api/?name=${customer.nama}&background=1e293b&color=fff`} alt={customer.nama} className="w-full h-full object-cover" />
                  </div>
                  <div>
                ...
                <div className="space-y-4">
                  <div className="flex items-start gap-3">
                    ...
                    <p className="text-sm text-gray-700 font-medium cursor-pointer hover:text-blue-600" onClick={handleWA}>{customer.noHp}</p>
                  </div>
                  <div className="flex items-start gap-3">
                    <MapPin size={16} className="text-gray-400 mt-0.5" />
                    <p className="text-sm text-gray-700">{customer.alamat || 'Alamat tidak diisi'}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white border border-gray-100 rounded-2xl shadow-sm p-6 relative">
                <div className="flex justify-between items-start mb-6">
                  <h2 className="text-sm font-semibold text-gray-900 flex items-center gap-2">
                    <MonitorSmartphone size={18} className="text-blue-500" /> Detail Perangkat
                  </h2>
                </div>
                ...
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900">{order.jenisPerangkat} {order.merkModel}</h3>
                    <p className="text-xs text-gray-500 mt-1 uppercase tracking-wider font-medium">SN: {order.noSeri || '-'}</p>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-3 mb-5">
                  <div className="bg-gray-50/80 p-3 rounded-xl border border-gray-100">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">KELUHAN</p>
                    <p className="text-sm font-semibold text-gray-800 leading-snug">{order.keluhan}</p>
                  </div>
                  <div className="bg-gray-50/80 p-3 rounded-xl border border-gray-100 relative group">
                    <p className="text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-1">PERKIRAAN</p>
                    <p className="text-sm font-semibold text-gray-800">Rp {order.estimasiBiaya.toLocaleString('id-ID')}</p>
                    {order.status !== 'DIAMBIL' && (
                      <button onClick={() => { setEditBiayaValue(order.estimasiBiaya.toString()); setIsEditBiayaOpen(true); }} className="absolute top-3 right-3 text-gray-300 hover:text-blue-600 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Edit2 size={12} />
                      </button>
                    )}
                  </div>
