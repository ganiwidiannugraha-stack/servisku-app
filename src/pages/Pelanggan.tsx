...
export const Pelanggan: React.FC = () => {
  ...
  const getCustomerTier = (total: number) => {
    if (total >= 10)
      return { type: "Gold", color: "text-yellow-500", icon: Trophy };
    if (total >= 5)
      return { type: "Silver", color: "text-gray-400", icon: Shield };
  ...
  const filteredCustomers = customers.filter((c) => {
    ...
    const matchesTier =
      tierFilter === "ALL" || tier.toUpperCase() === tierFilter;
    const activeOrders = getActiveOrdersCount(c.id);
    const matchesStatus =
      statusFilter === "ALL" ||
      (statusFilter === "ACTIVE" && activeOrders > 0) ||
      (statusFilter === "INACTIVE" && activeOrders === 0);
  ...
  const totalServisAll = customers.reduce(
    (sum, c) => sum + (c.totalServis || 0),
    0,
  );
  const rataRataServis =
    totalPelanggan > 0 ? (totalServisAll / totalPelanggan).toFixed(1) : "0";
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-gray-500 font-medium">
            Kelola data pelanggan dan tingkatkan loyalitas mereka.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 mb-10">
        ...
        <div className="lg:col-span-5 grid grid-cols-2 gap-4">
          ...
          <div className="bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col justify-center">
            <p className="text-sm font-bold text-gray-900 mb-1">
              Pelanggan Baru{" "}
              <span className="text-gray-500 font-normal">(Bulan Ini)</span>
            </p>
        ...
        <div className="lg:col-span-7 bg-white rounded-2xl p-6 border border-gray-100 shadow-sm flex flex-col relative overflow-hidden">
          <p className="text-sm font-bold text-gray-900 mb-6">
            ...
            <span className="text-gray-500 font-normal">(Tahun Ini)</span>
          </p>
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center gap-3 min-w-[140px]">
                  <div
                    className={`p-2 rounded-full ${isGold ? "bg-yellow-50" : tier.type === "Silver" ? "bg-gray-50" : "bg-orange-50"}`}
                  >
                ...
                <div className="min-w-[120px]">
                  ...
                  <p className="font-bold text-gray-900 text-sm">
                    {new Date(customer.terakhirServis).toLocaleDateString(
                      "id-ID",
